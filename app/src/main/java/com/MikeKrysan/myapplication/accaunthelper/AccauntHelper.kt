...
//            Log.d("MyLog", "Exception2: ${exception.errorCode}")
    ...
    private fun getSignInClient(): GoogleSignInClient {  //8.2 функция должна вернуть googleSignInClient. Но мы не можем получить все необходимые классы из библиотеки, потому как у нас нет этой библиотеки. Нужно ее добавить
        ...
        val gso = GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
            ...
            .build()     //8.4 запрашиваем и токен и имейл
        ...
        user.sendEmailVerification().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                Toast.makeText(
                    ...
                    .show()  //Всплывает тост, что пользователю на почту отправлено уведомление для подтверждения регистрации
            } else {
                Toast.makeText(
                    act,
                    act.resources.getString(R.string.send_verification_error),
                    Toast.LENGTH_LONG
                ).show()    //Показываем, что ошибка отправки подтверждения на почту
            }
        }
    }
